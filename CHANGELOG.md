...
- 修复模型配置表单在编辑时被异步状态恢复覆盖导致数据丢失的问题，现在只在初次加载时填充表单
- 修复开放世界和分支互动(play)功能调用错误模型的问题，为 play agents 添加正确的 Agent 路由 fallback 配置
- 为项目设置添加"互动"Agent 路由选项，允许用户为开放世界场景单独配置模型
- 改善状态修复(state-repair)流程的 Agent 路由，确保使用用户配置的写作模型而非默认模型
