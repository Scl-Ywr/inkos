...
export function ServiceDetailPage({ serviceId, nav }: { serviceId: string; nav: Nav }) {
  ...
  useEffect(() => {
    ...
    void fetchJson<{ services: Array<Record<string, unknown>>; service?: string | null; defaultModel?: string | null }>("/services/config")
      .then((data) => {
        ...
        if (data.service === effectiveServiceId && data.defaultModel) {
          setDetectedModel(data.defaultModel);
        }
      })
      .catch(() => {});
    return () => { cancelled = true; };
  }, [isCustom, persistedCustomName, serviceId, effectiveServiceId]);
  ...
  return (
    <div className="max-w-xl mx-auto space-y-6">
      ...
      <div className="paper-sheet rounded-xl p-6 md:p-8 space-y-6">
        {/* Custom fields */}
        {isCustom && (
