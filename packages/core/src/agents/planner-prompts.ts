...
Produce the memo for chapter {{chapterNumber}}. The YAML frontmatter must contain exactly chapter: {{chapterNumber}}. Strictly emit YAML frontmatter + markdown.`;
...
请为第 {{chapterNumber}} 章产生 memo。YAML frontmatter 必须精确包含 chapter: {{chapterNumber}}。严格按 YAML frontmatter + markdown 格式输出。`;
